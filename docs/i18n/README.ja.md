...
### 2. AIをどこで動かすか選ぶ(そのままでも動きます)

既定では**Chrome内蔵のGemini Nano**を使います。何もインストールせず、サーバーも立てずに使えます。初回チェック時にモデルが一度だけダウンロードされます(Chrome 138以降・空き容量約22GBが必要)。動かせない環境ではパネルがその旨を伝えるので、バックエンドを切り替えてください。

もっと大きいモデルや好みのモデルを使いたい場合は、拡張の**オプション**でバックエンドを**ローカルサーバー**に切り替え、自分で動かすOpenAI互換エンドポイントを指定します。
...
どちらの場合も、メンテナが費用を払うことも文章を見ることもありません。文章はあなたのPC内にとどまります。
...
`<textarea>`、1行のテキスト`<input>`、`contenteditable`なエディタ(Gmailなど)で動きます。contenteditableへの書き戻しはエディタ自身の挿入コマンド経由で行うため、周囲の書式や取り消し履歴(Undo)が保たれます。
...
Harperは競合というより**補完関係**で、inline-scribeはそれを直接使えます。任意の[Harper pre-pass](#設定)をONにすると、機械的で決定的な修正はHarperが瞬時に潰し、ルールベースでは無理な書き直しだけをローカルLLMが担当します。どちらも端末内で完結します。
...
入力欄でAlt+G
   │
   ▼ 文章を、自分のPC内で動くAIに渡す
     (既定はChrome内蔵のGemini Nano。インストール不要)
     (バックエンドを切り替えれば自前のOllamaなどに送ることも可)
   │
   ▼ モデルは「直した後の文章」だけを返す
   │
...
3. **決定的な作業は決定的なエンジンへ(任意)。** Harper pre-passをONにすると、機械的な修正はモデルの前にHarperのルールベースエンジンが処理するので、LLMは判断が要る部分だけに集中できます。HarperのWASMは端末内で動き、pre-passをONにしたときだけ読み込まれます。
...
- **Backend** — **Chrome内蔵AI(Gemini Nano)**(既定。インストール不要)か、**ローカルサーバー**(自前のエンドポイント)。レビューUIはどちらでも同じです
- **Endpoint**(ローカルサーバー選択時のみ) — OpenAI互換APIなら何でも。Ollama / llama.cpp / LM Studio / vLLM、自分のAPIキーでクラウドに繋ぐのも可。既定は`http://127.0.0.1:11434/v1`
- **Model**(ローカルサーバー選択時のみ) — 既定`llama3.2`。大きいモデルに替えれば提案の質だけが上がります
...
- **Harper pre-pass**(任意・既定OFF) — ONにすると、高速・ルールベース・完全ローカルの文法エンジン[Harper](https://writewithharper.com)をAIの**前**に走らせます。機械的なミス(大文字化・約物・スペース・主述の一致・単語の重複)はHarperが瞬時かつオフラインで直し、AIは文章の流れや言い回しだけを担当します。綴りの推測(スペルミス・typo)は文脈を持つAIに意図的に委ねます。HarperはWASMとして端末内で動くので、これも100%ローカルです
...
- **既定のバックエンド**ではモデルが端末内で動く(Chrome内蔵のGemini Nano)ため、文章はPCの外に一切出ません。**ローカルサーバー**バックエンドの場合は、自分で設定したエンドポイントだけに送られます
- アカウント登録なし、アナリティクスなし、テレメトリなし。保存されるのは設定値だけ(`chrome.storage.sync`)
- このプロジェクトにサーバーはありません。メンテナが費用を払うことも、あなたの文章を見ることもできない構造です

## 今後

- Chrome内蔵の**Proofreader API**(Gemini Nano)対応 — 校正に特化した端末内バックエンドの選択肢として、origin trialを抜けたら同じレビューUIの裏に追加(現在の端末内既定はGAされたPrompt API)
- Firefox対応
